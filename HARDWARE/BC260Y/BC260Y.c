...
void MQTT_joint(void)
{
    ...
    sprintf(BC260_Massage.AT_MQTT_PUB_WEIGHT, AT_MQTT_PUB_WEIGHT_REPORT,BC260_Massage.BC260_SN); //接收时间同步 主题
...
//       xSemaphoreTake(MutexSemaphore,portMAX_DELAY);	//获取互斥信号量
        ...
        {                       
			if( Send_Timeout>=120 )//超过15S对广播一次 定时器500Ms加一
            {
                ...
                sprintf(JSON_MASSAGE,JSON_BC260Y_MASSAGE,BC260_Massage.BC260_SN,BC260_Massage.BC260_IMEI, 1 ,1, xTaskGetTickCount());//xTaskGetTickCount():获取时间戳                
               // printf("size=%d\r\n",strlen(JSON_MASSAGE));
             ...
             if(Pcd_Massage_Flag.Pcd_Read_Flag==1 && Pcd_Massage_Flag.Pcd_Write_Flag==2 )       //Pcd_Write_Flag 在完成写卡后置2
            {
                ...
                sprintf(t_json, json, Pcd_Massage_Flag.Pcd_Card_ID , HX711_Massage.Write_To_Card_Weight);
			...
			if(Send_timing_wakeup>=20)              //定时发送，维持模块唤醒
            {
                Send_timing_wakeup=0;  
                char res[]="OK"; 
            ...
            if(Pcd_Massage_Flag.Pcd_Legal_Flag==2 &&  BC260_Massage.send_error==1)  //如果重量不合法，上报
            {
                memset(&t_json,0,sizeof(t_json));
                char PUB_WEIGHT[]="{\"sn\":\"%s\"}";                                
                sprintf(t_json, PUB_WEIGHT, BC260_Massage.BC260_SN);
