...
uint8_t ParseStr(const char *JSON,TPYE CJSON_TASK_TPYE)
{
    ...
    else
    {
        ...
        break;
        case createRTCTask:
            if(strstr(str_json->valuestring,"arder")!=0){
        ...
        case createClearTask:
            ...
            W25QXX_Write(&BC260_Massage.bindFlag,BindFlag_DATA_FLASH_BASE,BindFlag_DATA_SIZE);//绑定状态置0，并保存
        //    memset(&work_time,0,sizeof(work_time));//每次删除操作后都要复位重新判断
        break;
        case createBindTask:
                BC260_Massage.bindFlag=1;
                W25QXX_Write(&BC260_Massage.bindFlag,BindFlag_DATA_FLASH_BASE,BindFlag_DATA_SIZE); //绑定设备 并保存
        break;
        default:
            break;
        } 
