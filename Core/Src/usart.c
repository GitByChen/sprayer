...
uint16_t UART1_RX_STA=0;    // �?15bit表示�?帧数据接收完成，�?14~0位表示接收到的数据量
//uint16_t UART2_RX_STA=0;    // �?15bit表示�?帧数据接收完成，�?14~0位表示接收到的数据量
uint16_t UART3_RX_STA=0;    // �?15bit表示�?帧数据接收完成，�?14~0位表示接收到的数据量
...
{
	u8 Res;
	if(__HAL_UART_GET_FLAG(&huart1, UART_FLAG_RXNE) != RESET)  // 空闲中断标记被置�?
	{	
    __HAL_UART_DISABLE_IT(&huart1, UART_IT_RXNE);
		...
		if(__HAL_UART_GET_FLAG(&huart1, UART_FLAG_IDLE) != RESET)  // 空闲中断标记被置�?
	{
...
//{
//	u8 Res;
//	if(__HAL_UART_GET_FLAG(&huart2, UART_FLAG_RXNE) != RESET)  // 空闲中断标记被置�?
//	{	
...
//		}
//	}
//		if(__HAL_UART_GET_FLAG(&huart2, UART_FLAG_IDLE) != RESET)  // 空闲中断标记被置�?
//	{
//			UART2_RX_STA|=0x8000;
...
//	}
//			if(__HAL_UART_GET_FLAG(&huart2, UART_FLAG_ORE) != RESET)  // 空闲中断标记被置�?
//			{
...
void USART3_IRQHandler(void)
{
	...
	if(__HAL_UART_GET_FLAG(&huart3, UART_FLAG_RXNE) != RESET)  // 空闲中断标记被置�?
	{	
		...
		if(__HAL_UART_GET_FLAG(&huart3, UART_FLAG_IDLE) != RESET)  // 空闲中断标记被置�?
	{
			UART3_RX_STA|=0x8000;
